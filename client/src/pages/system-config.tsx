...
export default function SystemConfigPage() {
  ...
  useEffect(() => {
    setMounted(true);
    console.log("SystemConfigPage montado");
  ...
  if (error) {
    return (
      <div className="p-4">
        ...
        <pre className="mt-4 p-2 bg-muted rounded overflow-auto">
        ...
        {configs.map((config) => (
          <Card key={config.key} className="shadow-md">
            ...
            <CardContent>
              ...
              <div className="text-sm text-muted-foreground mt-2">Clave: {config.key}</div>
            </CardContent>
          </Card>
        ))}
      </div>
