...
type UserRoleRuleValues = z.infer<typeof userRoleRulesSchema>;
...
export default function BookingRulesPage() {
  ...
  const [editingRoleRule, setEditingRoleRule] = useState<UserRoleRuleValues | null>(null);
  
  // Estado para las reglas por rol de usuario
  const [roleRules, setRoleRules] = useState<UserRoleRuleValues[]>([
    {
      role: "standard",
  ...
  // Formulario para editar reglas por rol de usuario
  const roleRuleForm = useForm<UserRoleRuleValues>({
  ...
  // Actualizar formulario de reglas por rol cuando se edita
  useEffect(() => {
    if (editingRoleRule) {
      roleRuleForm.reset(editingRoleRule);
    }
  }, [editingRoleRule, roleRuleForm]);
  ...
  const handleEditRoleRule = (rule: UserRoleRuleValues) => {
  ...
  const handleSaveRoleRule = () => {
    const values = roleRuleForm.getValues();
    
    if (editingRoleRule) {
      const updatedRules = roleRules.map(rule => 
        rule.role === values.role ? values : rule
      );
      ...
      toast({
        title: "Reglas actualizadas",
        description: `Las reglas para usuarios ${values.role} han sido actualizadas.`,
      });
    }
  };
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      {/* Modal para edición de reglas por rol */}
      <Dialog open={isRoleRuleModalOpen} onOpenChange={setIsRoleRuleModalOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Editar Reglas para {editingRoleRule?.role}</DialogTitle>
            <DialogDescription>
              Ajuste las reglas específicas para este tipo de usuario.
            </DialogDescription>
          </DialogHeader>
          <Form {...roleRuleForm}>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between p-4 border rounded-md">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          Los usuarios podrán pagar una mensualidad para tener acceso ilimitado
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between p-4 border rounded-md">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          Permitir que el usuario reserve varias horas seguidas
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsRoleRuleModalOpen(false)}>Cancelar</Button>
            <Button onClick={handleSaveRoleRule}>Guardar cambios</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
