...
export default function BookingModal({
  ...
  // Usar una fecha válida, pero nunca sobreescribir la fecha seleccionada si existe
  const actualDate = selectedDate || new Date();
  console.log("BookingModal renderizado", { isOpen, selectedRoomId, selectedDate, actualDate });
  ...
  // Crear fecha del sistema actual para valores iniciales
  const todayDate = new Date();
  console.log("BookingModal - Fecha actual del sistema:", todayDate);
  ...
  useEffect(() => {
    if (isOpen) {
      // Crear nueva fecha actual para asegurar que siempre tenemos una fecha válida
      const currentDate = new Date();
      
      console.log("Reseteando formulario con valores", { 
        selectedRoomId, 
        selectedDate, 
  ...
  const createBookingMutation = useMutation({
    mutationFn: async (bookingData: any) => {
      const result = await createBookingSafe(bookingData, (opts) => toast(opts));
  ...
  const onSubmit = async (values: BookingFormValues) => {
    ...
    try {
      ...
      // Nos aseguramos de que estamos usando la fecha correcta
      // Si vino una fecha seleccionada de los parámetros, usamos esa
      // Si no, usamos la fecha del formulario
      // Y siempre la pasamos por formatDateForAPI para garantizar el formato correcto
      const dateToUse = values.date;
      const correctedDate = formatDateForAPI(dateToUse);
      
      console.log("Fecha original:", dateToUse);
      console.log("Fecha del formulario (año/mes/día):", 
      ...
      // Obtenemos información del precio
      const roomDetails = rooms?.find(r => r.id === roomId);
      const roomCost = roomDetails?.price || 0;
  ...
  const checkRoomAvailability = async (roomId: number, date: Date, startTime: string, endTime: string) => {
    try {
      ...
      // Usamos nuestra función mejorada
      const bookingDate = formatDateForAPI(date);
      console.log(`Verificando disponibilidad para: Consultorio ${roomId}, Fecha ${bookingDate}, Hora ${startTime}-${endTime}`);
      console.log("Fecha del calendario (año/mes/día):", date.getFullYear(), date.getMonth() + 1, date.getDate());
